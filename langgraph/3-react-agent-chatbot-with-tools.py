...
graph_builder.add_edge("tools", "tools_calling_model")
...
second_response = graph.invoke(State(messages = [HumanMessage(content="Multiply 2 by 3")]))
print(second_response["messages"][-1])

third_response = graph.invoke(State(messages = [HumanMessage(content="What is the latest news in ai and then multiply 2 by 3")]))
print(third_response["messages"][-1])